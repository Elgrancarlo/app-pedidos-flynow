...
function OfertasProdutoInline({ grupo }: { grupo: EstoqueProdutoResumo }) {
  return (
    <div className="px-3 pb-4 pt-1">
      <div className="overflow-hidden rounded-[8px] border border-[var(--fly-border)] bg-[var(--fly-surface)] shadow-[var(--fly-panel-inset)]">
        <div className="grid gap-2 border-b border-[var(--fly-divider)] p-3 sm:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-[8px] bg-[var(--fly-surface-muted)] px-3 py-2.5">
            ...
            >
              {formatNumber(grupo.estoque_atual)} potes
            </p>
          </div>
          <div className="rounded-[8px] bg-[var(--fly-surface-muted)] px-3 py-2.5">
            <p className="text-[10px] font-semibold uppercase text-[var(--fly-text-muted)]">
            ...
            <p className="mt-1 text-base font-semibold tabular-nums text-[var(--fly-text)]">
              {formatNumber(grupo.vendasPeriodo)} potes
            </p>
          </div>
          <div className="rounded-[8px] bg-[var(--fly-surface-muted)] px-3 py-2.5">
            <p className="text-[10px] font-semibold uppercase text-[var(--fly-text-muted)]">
            ...
            <p className="mt-1 text-base font-semibold tabular-nums text-[var(--fly-text)]">
              {formatNumber(grupo.entradasPeriodo)} potes
            </p>
          </div>
          <div className="rounded-[8px] bg-[var(--fly-surface-muted)] px-3 py-2.5">
            <p className="text-[10px] font-semibold uppercase text-[var(--fly-text-muted)]">
          ...
          <table className="w-full min-w-[760px] table-fixed text-left text-sm">
            <thead>
              <tr className="border-b border-[var(--fly-divider)] bg-[var(--fly-table-head)] text-[11px] font-semibold uppercase text-[var(--fly-text-muted)]">
                ...
                <th className="w-[13%] px-3 py-3 text-right">Potes/pedido</th>
                <th className="w-[14%] px-3 py-3 text-right">
                  Potes vendidos
                </th>
                <th className="w-[14%] px-3 py-3 text-right">Última venda</th>
              </tr>
            </thead>
                ...
                >
                  <td className="px-3 py-3.5 align-middle">
                    ...
                    <span className="mt-1 block text-xs text-[var(--fly-text-muted)]">
                      Participação{" "}
                      {formatNumber(oferta.participacaoPeriodo * 100, 1)}% no
                      consumo do período
                    </span>
                  </td>
                ...
                grupos.map((grupo) => {
                  ...
                  return (
                    <Fragment key={grupo.id}>
                      ...
                      {isSelected ? (
                        <tr className="bg-[var(--fly-brand-soft)]">
                          <td colSpan={5} className="p-0">
                            <OfertasProdutoInline grupo={grupo} />
                          </td>
                        </tr>
                      ) : null}
                    </Fragment>
