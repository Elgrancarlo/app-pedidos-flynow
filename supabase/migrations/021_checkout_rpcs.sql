-- Migration 021: RPCs otimizadas para checkout monitor
-- Resolve gargalo de performance: de ~40s (fetch 10K rows + JS) para <500ms (SQL agregado)

-- ═══════════════════════════════════════════════════════════════════════
-- Índices adicionais para as queries otimizadas
-- ═══════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════
-- RPC: checkout_summary_counts (para widget do dashboard)
-- Retorna apenas os 4 contadores + totalEvents — sem rows
-- ═══════════════════════════════════════════════════════════════════════
...
AS $$
  ...
  recovered AS (
    ...
    WHERE l.event_status = 'paid'
      AND EXISTS (
        SELECT 1 FROM payt_event_stream es
...
-- ═══════════════════════════════════════════════════════════════════════
-- RPC: checkout_monitor_summary (para /carrinhos — lista completa paginada)
-- Replica exatamente a lógica de getPaytCheckoutMonitor() do JS
-- ═══════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Último evento por entidade (cart/transaction)
  ...
  -- Timeline (lista de status) e contagem de eventos por entidade
  timelines AS (
    SELECT
      entity_key,
  ...
  -- Apenas rows não-pagas em grupos relevantes (para a tabela)
  filtered_rows AS (
    SELECT
      l.*,
      t.timeline,
